...
class ScaledDotProductAttentionWithGrid(nn.Module):
    '''
    Scaled dot-product attention
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, comment=None):
        ...
        :param h: Number of heads
        '''
        super(ScaledDotProductAttentionWithGrid, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        self.fc_o = nn.Linear(h * d_v, d_model)
        ...
        self.d_v = d_v
        self.h = h
        # print('k, v, h', d_k, d_v, h)
        self.init_weights()

        self.comment = comment

        # self.mlp = nn.Sequential(
        #     nn.LayerNorm(f_obj), nn.Linear(d_model, f_out), nn.Dropout(p=dropout)
        # )
        self.d_g = d_k
        self.fc_g0 = nn.Linear(2048, h * d_k)
        self.fc_g = nn.Linear(49, 8)
    ...
    def forward(self, queries, keys, values, grids, attention_mask=None, attention_weights=None):
        '''
        Computes
        :param queries: Queries (b_s, nq, d_model)
        :param keys: Keys (b_s, nk, d_model)
        ...
        q = self.fc_q(queries)
        # print('q shape after fc:', q.shape)
        ...
        # print('mul q and k:',torch.matmul(q, k).shape)

        # print('grids shape origin', grids.shape) # torch.Size([50, 49, 2048])
        g = self.fc_g0(grids)
        # print('grids shape after fc_g0', g.shape)
        b_g, ng = g.shape[:2]
        g = g.view(b_g, ng, self.h, self.d_g).permute(0, 2, 1, 3) # (50, 8, 49, 64)
        if b_s != b_g:
            g = g.permute(3, 1, 2, 0) # (64, 8, 49, 50)
            temp0 = torch.ones(size=[self.d_g, self.h, b_g, b_s]).to('cuda')
            g = torch.matmul(g, temp0)
            g = g.permute(3, 1, 2, 0)

        # change g
        g = g.permute(0, 1, 3, 2)
        temp = torch.ones(size= [b_s, self.h, ng, nq]).to('cuda')
        g = torch.matmul(g, temp)
        g = g.permute(0,1,3,2)
        g = torch.log(torch.clamp(g, min=1e-6))
...
class MultiHeadAttentionWithGrid(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
                 attention_module=None, attention_module_kwargs=None, comment=None):
        super(MultiHeadAttentionWithGrid, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.attention = ScaledDotProductAttentionWithGrid(d_model=d_model, d_k=d_k, d_v=d_v, h=h, comment=comment)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, queries, keys, values, grids, attention_mask=None, attention_weights=None):
        ...
        if self.identity_map_reordering:
            ...
            k_norm = self.layer_norm(keys)
            v_norm = self.layer_norm(values)
            g_norm = self.layer_norm(grids)
            out = self.attention(q_norm, k_norm, v_norm, g_norm, attention_mask, attention_weights)
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(queries, keys, values, grids, attention_mask, attention_weights)
            out = self.dropout(out)
            out = self.layer_norm(queries + out)
        return out